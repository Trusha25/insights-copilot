...
export default function Sidebar({ currentView, onNavigate, onNewAnalysis }) {
  ...
  return (
    <div
      className={`bg-[var(--bg-surface)] border-r border-[var(--border-subtle)] h-screen flex flex-col sticky top-0 shrink-0 font-sans z-30 transition-all duration-300 ease-in-out shadow-sm ${
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-start gap-3 px-3.5 py-2.5 rounded-xl transition-all duration-200 font-semibold text-sm cursor-pointer relative ${
                  isActive
                    ? 'bg-[var(--color-accent-bg)] text-[var(--color-accent)] border border-[var(--border-subtle)] shadow-xs'
      ...
      <div className={`border-t border-[var(--border-subtle)] shrink-0 ${isCollapsed ? 'p-3' : 'p-4'}`}>
        {isCollapsed ? (
          ...
          <div className="space-y-3">
            {/* Profile Row */}
            <div className="flex items-center justify-between gap-2.5 p-2.5 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)] hover:border-[var(--border-strong)] transition-all group">
              <div className="flex items-center gap-2.5 min-w-0">
                {/* Avatar with gradient ring */}
                <div className="relative shrink-0">
                  <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-[var(--accent-start)] via-[var(--accent-mid)] to-[var(--accent-end)] text-white flex items-center justify-center font-extrabold text-xs relative z-10 shadow-xs">
                    {getInitials()}
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold theme-text-title truncate" title={displayName || email}>
                    {displayName || email.split('@')[0]}
                  </p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-[9px] theme-text-muted font-semibold leading-none">Pro Plan</span>
                    <span className="inline-block bg-[var(--color-accent-bg)] text-[var(--color-accent)] border border-[var(--border-subtle)] text-[8px] font-black px-1.5 py-0.5 rounded-md leading-none">PRO</span>
                  </div>
                </div>
              </div>
              <button
                ...
                aria-label="Sign Out"
                title="Sign Out"
                className="p-1.5 rounded-lg theme-text-muted hover:theme-text-title hover:bg-[var(--bg-primary)] transition-all shrink-0 cursor-pointer"
              >
            ...
            <div className="p-2.5 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)] space-y-2">
              ...
              <div className="w-full bg-[var(--bg-primary)] rounded-full h-1.5 overflow-hidden relative border border-[var(--border-subtle)]">
                <div
