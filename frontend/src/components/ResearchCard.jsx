...
function ExpandableListItem({ children, textToQuery, idea, research, plan, workspaceId }) {
  ...
  return (
    <li className="list-none">
      <div 
        className="flex items-start gap-2 cursor-pointer hover:text-[var(--color-accent)] transition-colors"
        onClick={toggleExpand}
      >
        <svg 
          className={`w-4 h-4 shrink-0 mt-1 transition-transform ${isExpanded ? 'rotate-90 text-[var(--color-accent)]' : 'text-slate-400'}`} 
      ...
      {isExpanded && (
        <div className="mt-3 ml-6 mb-2 p-4 bg-[var(--color-accent-bg)] border border-[var(--color-border)] rounded-xl text-sm leading-relaxed theme-text-body shadow-inner">
          {isLoading ? (
            ...
            <div className="space-y-3">
              ...
              {mentorResponse.learning_resources && mentorResponse.learning_resources.length > 0 && (
                <div className="pt-2 mt-2 border-t border-[var(--color-border)]">
                  <h6 className="font-semibold theme-text-title mb-1">Learn More:</h6>
...
export default function ResearchCard({ status, research, critique, idea, plan, workspaceId, onRefreshSuccess }) {
  ...
  return (
    ...
    <div className="space-y-6">
      <div>
        <h4 className="font-bold text-lg theme-text-title mb-2">Problem Overview</h4>
        <p className="text-base theme-text-body leading-relaxed">
          {research.problem_validation || "N/A"}
        </p>
      </div>

      <div>
        <h4 className="font-bold text-lg theme-text-title mb-2">Existing Solutions</h4>
        {research.existing_solutions && research.existing_solutions.length > 0 ? (
          <ul className="space-y-3">
            {research.existing_solutions.map((sol, idx) => (
              ...
              >
                <div className="text-base theme-text-body">
                  <span className="font-bold theme-text-title">{sol.name}</span>: {sol.description}
                  {sol.gap && <div className="text-sm theme-text-muted mt-0.5">{sol.gap}</div>}
                </div>
              </ExpandableListItem>
            ))}
          </ul>
        ) : (
          <p className="text-base theme-text-body">No direct named competitors found.</p>
        )}
      </div>

      <div>
        <h4 className="font-bold text-lg theme-text-title mb-2">Market Opportunity</h4>
        <p className="text-base theme-text-body leading-relaxed">
      ...
      <div>
        <h4 className="font-bold text-lg theme-text-title mb-2">Research Gaps</h4>
          ...
          <ul className="space-y-3">
            {research.research_gaps.map((gapObj, idx) => (
              <ExpandableListItem 
                ...
                idea={idea} research={research} plan={plan} workspaceId={workspaceId}
              >
                <div className="text-base theme-text-body">{gapObj.gap}</div>
              </ExpandableListItem>
            ))}
          </ul>
        ) : (
          <p className="text-base theme-text-body">No significant research gaps identified.</p>
        )}
      </div>

      <div>
        <h4 className="font-bold text-lg theme-text-title mb-2">Innovation Opportunities</h4>
        {research.innovation_opportunities && research.innovation_opportunities.length > 0 ? (
          <ul className="list-disc pl-6 text-base theme-text-body space-y-2 marker:text-[var(--color-accent)]">
            {research.innovation_opportunities.map((opp, idx) => (
              <li key={idx}>
                <span>{opp.approach}</span>
                <div className="text-sm theme-text-muted mt-0.5">Addresses: {opp.addresses}</div>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-base theme-text-body">No innovation opportunities identified.</p>
        )}
      </div>
        ...
        {research.unverified_claims && research.unverified_claims.length > 0 && (
          <div className="bg-[var(--color-accent-bg)] border border-[var(--color-border-hover)] p-4 rounded-xl">
            <h4 className="font-bold text-lg text-[var(--color-accent)] mb-2 flex items-center gap-2">
        ...
        <div>
          <h4 className="font-bold text-lg theme-text-title mb-3">Critique Breakdown</h4>
