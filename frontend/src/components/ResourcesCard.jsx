  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm h-fit">
      ...
      <div className="space-y-6">
                    ...
                    <div className="min-w-0">
                      {src.title && (
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className="text-sm font-semibold text-slate-800 group-hover:text-blue-700 line-clamp-1">
                            {src.title}
                          </p>
                          {newSourceUrls.includes(src.url) && (
                            <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 shrink-0">New</span>
                          )}
                        </div>
                      )}
        ...
        {/* APIs & Datasets */}
        <div>
          <h4 className="font-bold text-lg text-slate-800 mb-3">APIs & Datasets</h4>
            ...
            <div className="space-y-3">
              {research.apis_datasets.map((item, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block p-3 rounded-xl border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50 transition-colors group"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-semibold text-slate-800 group-hover:text-emerald-700 line-clamp-1">
                          {item.name || item.url}
                        </p>
                        {item.url && newSourceUrls.includes(item.url) && (
                          <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 shrink-0">New</span>
                        )}
                        <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${item.type === 'dataset' ? 'bg-purple-100 text-purple-700' : 'bg-emerald-100 text-emerald-700'}`}>
                          {item.type}
                        </span>
                      </div>
                      <p className="text-sm text-slate-500 line-clamp-1">{item.url}</p>
                    </div>
                  </div>
                </a>
              ))}
