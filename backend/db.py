...
def save_history(idea: str, result: dict, user_id: str = None):
    try:
        ...
        payload = {"idea": idea, "result": result}
        if user_id:
...
def create_workspace(workspace_id: str, idea: str, research: dict, plan: dict,
                     user_id: str = None, critique: dict = None):
    try:
        supabase = get_supabase()
...
def update_workspace_research(workspace_id: str, research: dict, user_id: str = None):
    try:
        ...
        update_query = supabase.table("workspaces").update({"research_json": updated_research}).eq("id", workspace_id)
        if user_id:
...
def update_workspace_chat_history(workspace_id: str, chat_history: list, user_id: str = None):
    try:
        supabase = get_supabase()
        try:
            query = supabase.table("workspaces").update({"chat_history": chat_history}).eq("id", workspace_id)
            if user_id:
                query = query.eq("user_id", user_id)
            query.execute()
        except Exception as col_err:
            logger.info(f"chat_history column not found, falling back to research_json: {col_err}")
            query = supabase.table("workspaces").select("research_json").eq("id", workspace_id)
            if user_id:
                query = query.eq("user_id", user_id)
            res = query.execute()
            if res.data:
                ...
                update_q = supabase.table("workspaces").update({"research_json": research}).eq("id", workspace_id)
                if user_id:
...
def mark_milestone_complete(workspace_id: str, user_id: str = None) -> dict:
    try:
        ...
        query = supabase.table("workspaces").select(
            "current_milestone_index, milestone_completions, plan_json"
        ).eq("id", workspace_id)
        if user_id:
            query = query.eq("user_id", user_id)
        res = query.execute()
...
def get_user_settings(user_id: str) -> dict:
    try:
        ...
        try:
            ...
            logger.warning(f"Could not insert user settings: {insert_err}")
...
def update_user_settings(user_id: str, theme: str, primary_model: str = None, experience_level: str = None) -> dict:
    try:
        ...
        try:
            ...
            logger.warning(f"Could not upsert user settings: {upsert_err}")
        ret = {"theme": theme}
        if primary_model:
